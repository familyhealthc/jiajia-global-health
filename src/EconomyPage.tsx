...
import { allTenors, sovereignYields, summaryTenors } from './sovereignYields';
...
const usValues = sovereignYields.find((item) => item.code === 'US')!.values;
const points = [1, 2, 3, 5, 7, 10, 20, 30].map((year) => ({ year, value: usValues[`${year}Y` as keyof typeof usValues]! }));
const markets = [
...
export default function EconomyPage({ locale }: { locale: Locale }) {
  ...
  return <main className="economy-page" lang={en ? 'en' : 'zh-CN'}>
    ...
    <section className="economy-comparison" aria-labelledby="comparison-heading">
      <div className="economy-section-title"><div><span>{en ? 'JIAJIA HEALTH / ECONOMIC OBSERVATORY' : '家家健康 / 经济与利率观察'}</span><h1 id="comparison-heading">{en ? 'Sovereign yields by country' : '各国国债收益率对比'}</h1></div><p>{en ? 'Five maturities · nine countries or regions · dated official snapshots, annual %' : '五种主要期限 · 九个国家或区域 · 官方日期快照，年化 %'}</p></div>
      <p className="economy-scroll-hint">{en ? "Swipe to see 2Y, 5Y, 10Y and 30Y →" : "向左滑动，查看 2、5、10、30 年 →"}</p><div className="economy-comparison-scroll"><table className="economy-comparison-table"><thead><tr><th scope="col">{en ? 'Country / region' : '国家 / 区域'}</th>{summaryTenors.map((tenor) => <th scope="col" key={tenor}>{tenor.replace('Y', en ? ' year' : ' 年')}</th>)}</tr></thead><tbody>{sovereignYields.map((item) => <tr key={item.code}><th scope="row"><span className="economy-flag" aria-hidden="true">{item.flag}</span><span className="economy-country-name">{item[locale]}<small>{item.date}</small></span></th>{summaryTenors.map((tenor) => <td key={tenor}>{item.values[tenor] === undefined ? <span className="economy-pending" title={en ? 'No verified matching observation' : '暂无可核对的同日数据'}>—</span> : `${item.values[tenor]!.toFixed(2)}%`}</td>)}</tr>)}</tbody></table></div>
      <div className="economy-comparison-meta"><p>{en ? 'Dates and yield methods differ across markets. Blank cells mean no verified matching observation; the euro area AAA curve is a regional reference. Scroll sideways on small screens.' : '各市场日期与收益率算法不同。空格表示尚无核实后的对应数据；欧元区 AAA 为区域参考曲线。小屏幕可横向滑动。'}</p><a href="#economy-sources">{en ? 'See dates and sources' : '查看逐国口径与来源'} <ArrowUpRight size={15}/></a></div>
    </section>

    <nav className="economy-quick-nav" aria-label={en ? 'On this page' : '本页导航'}><span>{en ? 'EXPLORE' : '快速跳转'}</span><a href="#macro-risk">{en ? 'Country data' : '国家数据'}</a><a href="#macro-history">{en ? 'Ten-year history' : '十年趋势'}</a><a href="#indicator-catalog">{en ? 'Full directory' : '618 项目录'}</a><a href="#data-quality">{en ? 'Data quality' : '数据口径'}</a></nav>

    <section className="economy-many-tenors" aria-labelledby="tenors-heading"><div className="economy-section-title"><div><span>{en ? 'FULL MATURITY MATRIX' : '完整期限矩阵'}</span><h2 id="tenors-heading">{en ? 'From three months to fifty years' : '从 3 个月到 50 年'}</h2></div><p>{en ? 'All officially verified maturity points · annual % · scroll horizontally' : '已核实的官方期限点 · 年化 % · 可横向滑动'}</p></div><div className="economy-many-scroll"><table><thead><tr><th scope="col">{en ? 'Market' : '市场'}</th>{allTenors.map((tenor) => <th scope="col" key={tenor}>{tenor}</th>)}</tr></thead><tbody>{sovereignYields.map((item) => <tr key={item.code}><th scope="row">{item.flag} {item[locale]}<small>{item.date}</small></th>{allTenors.map((tenor) => <td key={tenor}>{item.values[tenor] === undefined ? '—' : `${item.values[tenor]!.toFixed(2)}%`}</td>)}</tr>)}</tbody></table></div><p className="economy-method">{en ? 'A dash means no verified observation for that date and tenor. These curves mix spot, par, bill and benchmark bond yields; compare shapes within a market first. Germany and Canada use separate sources for the one-year point.' : '破折号表示该日期与期限尚无已核实数据。表内有即期、平价、国库券和基准债收益率；先看同一市场内部的曲线形状。德国、加拿大的 1 年端使用另一份官方来源。'}</p><a className="economy-matrix-source" href="#economy-sources">{en ? 'Method and source notes' : '查看口径与来源'} <ArrowUpRight size={14}/></a></section>

    <MacroRisk locale={locale} />
    <DataQuality locale={locale} />
    <MacroCatalog locale={locale} />

    <section className="economy-spreads" aria-labelledby="spread-heading"><div className="economy-section-title"><div><span>{en ? 'TERM SPREADS' : '期限利差'}</span><h2 id="spread-heading">{en ? 'How does the curve slope?' : '短端到长端，曲线怎么走？'}</h2></div><p>{en ? 'Differences within a market, in basis points (bp)' : '同一市场内部的期限差，单位：基点（bp）'}</p></div><div className="economy-spread-scroll"><table className="economy-spread-table"><thead><tr><th>{en ? 'Market' : '市场'}</th><th>10Y − 1Y</th><th>10Y − 2Y</th><th>30Y − 10Y</th></tr></thead><tbody>{sovereignYields.filter((item) => item.spreads).map((item) => <tr key={item.code}><th>{item.flag} {item[locale]}<small>{item.date}</small></th>{([['10Y','1Y'],['10Y','2Y'],['30Y','10Y']] as const).map(([long, short]) => { const a = item.values[long], b = item.values[short]; const bp = a === undefined || b === undefined ? undefined : Math.round((a-b)*100); return <td key={`${long}-${short}`}>{bp === undefined ? '—' : `${bp > 0 ? '+' : ''}${bp} bp`}</td>; })}</tr>)}</tbody></table></div><p className="economy-method">{en ? '1 bp = 0.01 percentage point. Only pairs from one official curve and observation date are shown. A spread describes curve shape, not an investment return.' : '1 bp = 0.01 个百分点。仅展示同一官方曲线、同一观察日的期限组合。期限利差描述曲线形状，不等于投资回报。'}</p></section>

    <section className="economy-workspace" aria-labelledby="yield-heading">
      <div className="economy-section-title"><div><span>{en ? 'UNITED STATES / NOMINAL' : '美国 / 名义收益率'}</span><h2 id="yield-heading">{en ? 'Treasury yield curve' : '美国国债期限曲线'}</h2></div><p>{en ? 'Daily Treasury par yield curve · September 23, 2026 · % per annum' : '美国财政部每日平价收益率曲线 · 2026 年 9 月 23 日 · 年化 %'}</p></div>
      <div className="economy-chart-layout">
        <div className="economy-chart" role="img" aria-label={en ? 'US Treasury yields from 1 to 30 years, September 22 2026' : '2026年9月23日美国1至30年期国债收益率曲线'}>
        ...
        <aside className="economy-readout" aria-live="polite"><span>{selected}{en ? '-YEAR TREASURY' : ' 年期美国国债'}</span><strong>{active.value.toFixed(2)}<small>%</small></strong><p>{en ? 'Par yield · Sep 23, 2026' : '平价收益率 · 2026-09-23'}</p><a href={source} target="_blank" rel="noopener noreferrer">{en ? 'View Treasury source' : '查看美国财政部原始表格'} <ArrowUpRight size={15}/></a></aside>
      </div>
    ...
    <section className="economy-sources" id="economy-sources"><div className="economy-section-title"><div><span>{en ? 'SOURCE NOTES' : '数据口径'}</span><h2>{en ? 'Check the original figures' : '查看原始数据与日期'}</h2></div></div><div className="economy-source-list">{sovereignYields.map((item) => <div key={item.code}><span>{item.flag} {item[locale]} <small>{item.date}</small></span><p>{item[en ? 'detailEn' : 'detailZh']}</p><div className="economy-source-links"><a href={item.source} target="_blank" rel="noopener noreferrer">{item.secondSource ? (en ? 'Main source' : '主要来源') : (en ? 'Official source' : '官方来源')} <ArrowUpRight size={14}/></a>{item.secondSource && <a href={item.secondSource} target="_blank" rel="noopener noreferrer">{en ? 'Additional source' : '补充来源'} <ArrowUpRight size={14}/></a>}</div></div>)}</div><p className="economy-method">{en ? 'Snapshots are for research and may be revised by the publisher. A sovereign yield is not an executable quote, deposit rate or business borrowing rate.' : '这些数值仅用于研究，发布机构可能修订。国债收益率不是可执行交易报价、存款利率或企业借款利率。'}</p></section>
